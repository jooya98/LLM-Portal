...
//
// Schema only — no catalog data. The rebuild also drops any child tables added
// after this migration (e.g. media_fallback_overrides, embedding_fallback_overrides),
// so it cannot run if they exist.
...
export function down(db: Db): void {
  ...
  // The rebuild in down() drops any child tables added after this migration,
  // so refuse to run if they exist.
  const laterChildren = ['media_fallback_overrides', 'embedding_fallback_overrides'];
  const existing = db.prepare(`
    SELECT name FROM sqlite_master
     WHERE type = 'table' AND name IN (${laterChildren.map(() => '?').join(', ')})
  `).all(laterChildren) as { name: string }[];
  if (existing.length > 0) {
    throw new Error(
      `Cannot revert per-endpoint model identity: the following tables were added after it and would be dropped: ${existing.map(t => t.name).join(', ')}. ` +
      'Revert the later migrations first.',
    );
  }
