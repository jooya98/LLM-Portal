...
export function groupRows(rows: GroupableRow[], ov: UnifyOverrides): ModelGroup[] {
  ...
  // A non-user-defined group with two members sharing BOTH platform and
  // endpoint_scope is never legitimate multi-provider redundancy — every real
  // multi-provider group has at most one row per (platform, endpoint_scope)
  // pair (custom relays legitimately share platform 'custom' and even the same
  // model_id, but are told apart by endpoint_scope, #651). A same-(platform,
  // endpoint_scope) duplicate only happens when an auto-derived display_name
  // coincidentally collides with an unrelated existing row. Split any such
  // duplicate (past the first, by lowest model_db_id) into its own singleton
  // group so it resolves and pins independently.
  for (const g of [...map.values()]) {
    if (g.userDefined) continue;
    const seenKeys = new Map<string, GroupableRow>();
    for (const m of [...g.members].sort((a, b) => a.model_db_id - b.model_db_id)) {
      const key = `${m.platform}::${m.endpoint_scope ?? ''}`;
      if (!seenKeys.has(key)) {
        seenKeys.set(key, m);
        continue;
      }
      g.members = g.members.filter(row => row.model_db_id !== m.model_db_id);
      const soloKey = `__solo__${key}::${m.model_id}`;
      map.set(soloKey, {
        groupKey: soloKey, canonicalId: '', groupLabel: stripProviderSuffix(m.display_name),
        members: [m], userDefined: false,
      });
    }
  }
