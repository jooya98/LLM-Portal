...
function resolveMediaChain(model: string | undefined, modality: MediaModality): MediaModelRow[] {
  ...
  const override = getDb().prepare(
    'SELECT chain_json FROM media_fallback_overrides WHERE media_model_db_id = ?'
  ).get(matches[0].id) as { chain_json: string } | undefined;
  if (override) {
    try {
      ...
      if (Array.isArray(parsedIds) && parsedIds.every(id => Number.isInteger(id) && id > 0)) {
        const existingIds = new Set(matches.map(row => row.id));
        for (const id of parsedIds as number[]) {
          if (existingIds.has(id)) continue;
          const row = getDb().prepare('SELECT * FROM media_models WHERE id = ? AND enabled = 1 AND modality = ?').get(id, modality) as MediaModelRow | undefined;
          if (row) {
            matches.push(row);
            existingIds.add(id);
          }
      ...
      // Ignore malformed override data and preserve the literal-match chain.
    }
  }
  return matches;
}
...
function resolveTranscriptionChain(model: string | undefined, responseFormat: string): SttCandidate[] {
  let rows = listMediaModels('transcription');
  if (rows.length === 0) {
  ...
  if (model && !TRANSCRIPTION_AUTO_IDS.has(model.toLowerCase())) {
    rows = rows.filter(r => r.model_id === model);
    if (rows.length === 0) {
      throw new MediaError(
    ...
    const override = getDb().prepare(
      'SELECT chain_json FROM media_fallback_overrides WHERE media_model_db_id = ?'
    ).get(rows[0].id) as { chain_json: string } | undefined;
    if (override) {
      try {
        const parsedIds: unknown = JSON.parse(override.chain_json);
        if (Array.isArray(parsedIds) && parsedIds.every(id => Number.isInteger(id) && id > 0)) {
          ...
          for (const id of parsedIds as number[]) {
            ...
            const extra = getDb().prepare(
              "SELECT * FROM media_models WHERE id = ? AND enabled = 1 AND modality = 'transcription'"
            ).get(id) as MediaModelRow | undefined;
            if (extra) {
              rows.push(extra);
              existingIds.add(id);
            }
  ...
  let chain = rows.map(toSttCandidate);
