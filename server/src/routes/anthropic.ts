...
import { routeRequest, resolveStickyPreference, routingReserveTokens, resolveModelGroupCandidates, type RouteResult, type ChainRow } from '../services/router.js';
...
anthropicRouter.post('/messages', async (req: Request, res: Response) => {
  ...
  // A genuine pin (via the Claude family map or a direct catalog id) must not
  // silently fall back to the entire global auto-chain if it fails — only to
  // a manually configured chain, if one exists. `resolved.pinned` is false
  // for sticky-only preference, which correctly keeps using the full auto
  // chain since that isn't a hard pin.
  let anthropicChain: ChainRow[] | undefined;
  if (resolved.pinned && resolved.preferredModelDbId != null) {
    anthropicChain = resolveModelGroupCandidates([resolved.preferredModelDbId]);
    const override = getDb().prepare(
      'SELECT chain_json FROM model_fallback_overrides WHERE model_db_id = ?'
    ).get(resolved.preferredModelDbId) as { chain_json: string } | undefined;
    if (override) {
      try {
        const parsedIds: unknown = JSON.parse(override.chain_json);
        if (Array.isArray(parsedIds) && parsedIds.every(id => Number.isInteger(id) && id > 0)) {
          const overrideChain = resolveModelGroupCandidates(parsedIds as number[]);
          const existingIds = new Set(anthropicChain.map(row => row.model_db_id));
          for (const id of parsedIds as number[]) {
            if (existingIds.has(id)) continue;
            const row = overrideChain.find(candidate => candidate.model_db_id === id);
            if (row) {
              anthropicChain.push(row);
              existingIds.add(id);
            }
          }
        }
      } catch {
        // Ignore malformed override data and preserve the strict single-model chain.
      }
    }
  }
  ...
  await runFallbackLoop({
    ...
    route: () => routeRequest(estimatedTotal, state.skipKeys.size > 0 ? state.skipKeys : undefined, preferredModel, hasImage, wantsTools, state.skipModels.size > 0 ? state.skipModels : undefined, anthropicChain),
