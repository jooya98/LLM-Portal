...
proxyRouter.post('/completions', async (req: Request, res: Response) => {
  ...
  if (!isAutoModel(requestedModel) && requestedModel) {
    ...
    let members = resolved?.memberDbIds ?? null;
    if (resolved && members && resolved.demotedDbIds && resolved.demotedDbIds.size > 0 && resolved.demotedDbIds.size < members.length) {
      members = members.filter(id => !resolved.demotedDbIds.has(id));
    }
    if (members && members.length > 0) {
      ...
      ).get(members[0]) as { chain_json: string } | undefined;
      if (override) {
        try {
          ...
          if (Array.isArray(parsedIds) && parsedIds.every(id => Number.isInteger(id) && id > 0)) {
            ...
            const existingIds = new Set(groupChain.map(row => row.model_db_id));
          ...
          // Ignore malformed override data and preserve the resolved group chain.
        }
      }
    } else {
      ...
      if (enabled) {
        ...
        const override = db.prepare(
          'SELECT chain_json FROM model_fallback_overrides WHERE model_db_id = ?'
        ).get(preferredModel) as { chain_json: string } | undefined;
        if (override) {
          try {
            const memberDbIds: unknown = JSON.parse(override.chain_json);
            if (Array.isArray(memberDbIds) && memberDbIds.every(id => Number.isInteger(id) && id > 0)) {
              groupChain = resolveModelGroupCandidates(memberDbIds as number[]);
            }
          } catch {
            // Ignore malformed override data and preserve the existing fallback behavior.
          }
        }
      } else {
