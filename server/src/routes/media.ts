...
// Get fallback override chain for a media model
mediaRouter.get('/:id/fallback-override', (req: Request, res: Response) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id)) {
    res.status(400).json({ error: { message: 'Invalid id' } });
    return;
  ...
  const db = getDb();
  const row = db.prepare("SELECT chain_json FROM media_fallback_overrides WHERE media_model_db_id = ?").get(id) as { chain_json: string } | undefined;
  if (!row) {
    res.json({ chain: null });
    return;
  }

  res.json({ chain: JSON.parse(row.chain_json) });
});

// Update fallback override chain for a media model
const fallbackOverrideSchema = z.object({
  chain: z.array(z.number().int().positive()).min(1)
}).strict();

mediaRouter.put('/:id/fallback-override', (req: Request, res: Response) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id)) {
    res.status(400).json({ error: { message: 'Invalid id' } });
    return;
  ...
  const db = getDb();
  const pinnedModel = db.prepare("SELECT modality FROM media_models WHERE id = ?").get(id) as { modality: string } | undefined;
  if (!pinnedModel) {
    res.status(404).json({ error: { message: `Unknown media model ${id}` } });
    return;
  }

  // Validate all chain IDs exist in media_models and have the same modality
  const invalidIds: number[] = [];
  const wrongModalityIds: number[] = [];
  for (const chainId of parsed.data.chain) {
    const row = db.prepare("SELECT modality FROM media_models WHERE id = ?").get(chainId) as { modality: string } | undefined;
    if (!row) {
      invalidIds.push(chainId);
    } else if (row.modality !== pinnedModel.modality) {
      wrongModalityIds.push(chainId);
    }
  }

  if (invalidIds.length > 0) {
    res.status(400).json({ error: { message: `Invalid media model id(s) in chain: ${invalidIds.join(', ')}` } });
    return;
  }
  if (wrongModalityIds.length > 0) {
    res.status(400).json({
      error: {
        message: `Chain id(s) ${wrongModalityIds.join(', ')} are not '${pinnedModel.modality}' models. A fallback chain cannot mix modalities.`,
      },
    });
    return;
  }

  const chainJson = JSON.stringify(parsed.data.chain);
  db.prepare(`
    INSERT INTO media_fallback_overrides (media_model_db_id, chain_json)
    VALUES (?, ?)
    ON CONFLICT(media_model_db_id) DO UPDATE SET
...
// Delete fallback override chain for a media model
mediaRouter.delete('/:id/fallback-override', (req: Request, res: Response) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id)) {
    res.status(400).json({ error: { message: 'Invalid id' } });
    return;
  ...
  const db = getDb();
  db.prepare("DELETE FROM media_fallback_overrides WHERE media_model_db_id = ?").run(id);
  res.json({ success: true });
});
