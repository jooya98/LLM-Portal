...
// Get fallback override chain for a family
embeddingsRouter.get('/:family/fallback-override', (req: Request, res: Response) => {
  const family = decodeURIComponent(String(req.params.family));
  const db = getDb();

  const row = db.prepare('SELECT chain_json FROM embedding_fallback_overrides WHERE family = ?').get(family) as { chain_json: string } | undefined;
  ...
  try {
    ...
    res.status(500).json({ error: { message: 'Invalid chain JSON stored in database' } });
  }
});

// Update fallback override chain for a family
embeddingsRouter.put('/:family/fallback-override', (req: Request, res: Response) => {
  const family = decodeURIComponent(String(req.params.family));
  ...
  const familyExists = db.prepare('SELECT 1 FROM embedding_models WHERE family = ?').get(family);
  if (!familyExists) {
    res.status(404).json({ error: { message: `Unknown embedding family '${family}'` } });
    return;
  }

  // Check if all families in chain exist and that family doesn't fall back to itself
  const invalidFamilies: string[] = [];
  const allFamilies = new Set<string>();

  for (const f of parsed.data.chain) {
    if (f === family) {
      res.status(400).json({ error: { message: `Family '${family}' cannot fall back to itself` } });
      return;
    }

    if (allFamilies.has(f)) {
      continue; // Skip duplicate validation
    }
    allFamilies.add(f);

    const exists = db.prepare('SELECT 1 FROM embedding_models WHERE family = ?').get(f);
    if (!exists) {
      invalidFamilies.push(f);
    }
  }
  ...
  if (invalidFamilies.length > 0) {
    res.status(400).json({
      error: {
        message: `Unknown embedding families in chain: ${invalidFamilies.join(', ')}`
      }
    });
    return;
  }
  ...
  const upsert = db.prepare(`
    INSERT INTO embedding_fallback_overrides (family, chain_json)
    VALUES (?, ?)
...
// Delete fallback override chain for a family
embeddingsRouter.delete('/:family/fallback-override', (req: Request, res: Response) => {
  const family = decodeURIComponent(String(req.params.family));
  const db = getDb();

  db.prepare('DELETE FROM embedding_fallback_overrides WHERE family = ?').run(family);
