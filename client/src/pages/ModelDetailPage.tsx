...
import { ArrowDown, ArrowUp, ChevronLeft, Merge, Save, Split, Trash2, X } from 'lucide-react'
...
export default function ModelDetailPage() {
  ...
  const primaryModelDbId = members[0]?.modelDbId

  const { data: fallbackOverride } = useQuery<{ chain: number[] | null }>({
    queryKey: ['fallback-override', primaryModelDbId],
    queryFn: () => apiFetch(`/api/models/${primaryModelDbId}/fallback-override`),
    enabled: primaryModelDbId != null,
  })
  const { data: allModels = [] } = useQuery<{ id: number; displayName: string; platform: string; modelId: string }[]>({
    queryKey: ['models'],
    queryFn: () => apiFetch('/api/models'),
  })
  const setOverrideMutation = useMutation({
    mutationFn: (chain: number[]) =>
      apiFetch(`/api/models/${primaryModelDbId}/fallback-override`, { method: 'PUT', body: JSON.stringify({ chain }) }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['fallback-override', primaryModelDbId] }),
  })
  const clearOverrideMutation = useMutation({
    mutationFn: () => apiFetch(`/api/models/${primaryModelDbId}/fallback-override`, { method: 'DELETE' }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['fallback-override', primaryModelDbId] }),
  })
          ...
          <>
            ...
            <div className="rounded-2xl border bg-card p-4">
              ...
              <p className="mt-0.5 mb-3 text-xs text-muted-foreground">
                Tried in this exact order if the pinned model fails. Leave empty to error out instead of falling back.
              </p>
              ...
              {chainDraft.length > 0 && (
                <ol className="space-y-2 mb-4">
                  {chainDraft.map((modelId, index) => {
                    ...
                    return (
                      <li key={modelId} className="flex items-center gap-2 text-xs">
                        <span className="text-muted-foreground">{index + 1}.</span>
                        <span className="flex-1 rounded-full bg-muted px-2 py-0.5 text-xs">
              ...
              <div className="flex-1">
                <ModelCombobox
                  ...
                  onSelect={(value) => {
                    ...
                    if (selectedId && !chainDraft.includes(selectedId)) {
                      setChainDraft([...chainDraft, selectedId])
                    }
                  }}
                  ariaLabel="Select a model to add to the fallback chain"
