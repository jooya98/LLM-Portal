...
export function ProviderList({ onAddKey }: { onAddKey: () => void }) {
  ...
  const [discoverPlatform, setDiscoverPlatform] = useState<Platform | null>(null)
  ...
  return (
    <div>
            ...
            return (
              <div key={group.value}>
                <div className="flex items-center gap-2 pb-2">
                  ...
                  {(group.url || proxyEnabled || (group.value !== "custom" && group.keys.some(k => k.enabled))) && (
                    <DropdownMenu>
                      <DropdownMenuTrigger
                      ...
                      <DropdownMenuContent align="end" className="w-52">
                        ...
                        {group.value !== "custom" && group.keys.some(k => k.enabled) && (
                          <DropdownMenuItem onClick={() => setDiscoverPlatform(group.value)}>
                            {t('keys.discoverModels')}
                            <ListPlus className="ml-auto size-3.5" />
                          </DropdownMenuItem>
                        )}
                        {proxyEnabled && (
                          <DropdownMenuCheckboxItem
      ...
      {discoverPlatform !== null && (
        <DiscoverModelsDialog
          open
          onOpenChange={(open) => { if (!open) setDiscoverPlatform(null) }}
